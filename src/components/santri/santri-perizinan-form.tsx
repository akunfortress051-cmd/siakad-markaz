...
import TataTertibModal from "@/components/public/tata-tertib-modal";
import TasrihDigital from "@/components/public/tasrih-digital";
import Swal from "sweetalert2";

export default function SantriPerizinanForm({
  currentSantri,
  onSuccess
}: {
  currentSantri: SantriOption;
  onSuccess: () => void;
}) {
  ...
  useEffect(() => {
    ...
    // Set default selected santri for individu
    setSelectedSantri(currentSantri);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/santri/me/perizinan", {
        method: "POST",
      ...
      Swal.fire({
        title: "Terkirim!",
        text: "Permohonan izin berhasil dikirim. Silakan pantau status persetujuannya di halaman riwayat.",
        icon: "success",
  ...
  if (isSuccess) {
    return (
      <div className="py-12 animate-in fade-in zoom-in duration-500 flex flex-col items-center text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 max-w-sm">
            Izin telah disubmit. Anda dapat mengecek status setuju/tidaknya di tab Riwayat.
          </p>
        </div>
        <button 
          onClick={onSuccess}
          className="mt-4 px-6 py-3 bg-[var(--color-primary)] text-white shadow-lg rounded-xl font-bold hover:-translate-y-0.5 transition-all"
        >
          Lihat Riwayat Izin
        </button>
      </div>
